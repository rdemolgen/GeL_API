...
class AuthenticatedCIPAPISession(requests.Session):
    ...
    def authenticate_ad(self, testing_on=False):
        ...
        """
        #print("Authenticating using Active Directory")
        if testing_on == False:
    ...
    def authenticate_ldap(self):
        ...
        """
        #print("Authenticating using LDAP")
        try:
            token = (self.post(
...
class AuthenticatedOpenCGASession(requests.Session):
    ...
    def __init__(self):
        ...
        """
        requests.Session.__init__(self)
        #self.host_url = ('https://opencga.genomicsengland.nhs.uk/opencga/webservices/rest/v1')
        self.host_url = ('https://opencga-gms-beta.genomicsengland.nhs.uk/opencga/webservices/rest/v1')
        self.authenticate()
    ...
    def authenticate(self):
        ...
        """
        #print("Authenticating OpenCGA")
        opencga_auth_url = ('{host}/users/{username}/login'
                            .format(host=self.host_url,
                                    username=auth_credentials['username']))
        #print(opencga_auth_url)
        try:
            self.headers.update({"Accept": "application/json",
                                "Content-Type": "application/json",
                                "Authorization": "Bearer "})
            #data = '{"password":"password"}'
            #print(json.dumps(auth_credentials))
            sid_response = (self.post(opencga_auth_url,
                                        data=json.dumps({"password": auth_credentials['password']}))
                                .json())
